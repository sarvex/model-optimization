...
class Default8BitQuantizeRegistry(
  ...
  def __init__(self, disable_per_axis=False):
    self._layer_quantize_map = {
        quantize_info.layer_type: quantize_info
...
class Default8BitQuantizeConfigRNN(Default8BitQuantizeConfig, _RNNHelper):
  ...
  def get_weights_and_quantizers(self, layer):
    weights_quantizers = []
    for weight_attrs_cell, rnn_cell in zip(self.weight_attrs, self._get_rnn_cells(layer)):
  ...
  def get_activations_and_quantizers(self, layer):
    ...
    for activation_attrs_cell, rnn_cell in zip(self.activation_attrs, self._get_rnn_cells(layer)):
...
class Default8BitActivationQuantizeConfig(QuantizeConfig):
  ...
  def get_output_quantizers(self, layer):
    ...
    if not hasattr(layer.activation, '__name__'):
      raise ValueError(
          f'Activation {layer.activation} not supported by Default8BitActivationQuantizeConfig.'
      )
